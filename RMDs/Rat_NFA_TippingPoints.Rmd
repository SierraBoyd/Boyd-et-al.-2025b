---
title: "Calculation of Toxicological Tipping Points from Frank et al., 2018 rat NFA data"
author: "Sierra Boyd"
date: "2025-03-27"
...
project.input.dir <-"C:/Users/sboyd01/OneDrive - Environmental Protection Agency (EPA)/Profile/Documents/Project-Chronic DNT Organoid/Data Analysis/Tipping Points/CF Tipping Points/SourceData"
...
#filte to only include chemicals in common 
...
#mutate trt column to make dose=0 values to be called controls 
full_data<-full_data%>%mutate(trt = ifelse(dose == 0, "Control", trt))

#check unique chemicals
unique(full_data$trt)

#select only the columns of interest (identifers and endponits of interest)
full_data <-full_data%>%select(c("date","Plate.SN","DIV","well","trt","dose","units","meanfiringrate","burst.per.min","nAE","nABE","per.spikes.in.burst","r","mi"))

#Check for duplicates and remove
full_data<-full_data%>%distinct(date,Plate.SN,DIV,well,trt,dose,units,.keep_all=TRUE)

#Check that dose is numeric
full_data$dose<-as.numeric(full_data$dose)

#remove any NAs in the firing rate column-if NA this was likely a poor well that did not have any electrical activity
full_data<-full_data %>% drop_na(meanfiringrate)

#set as a data frame
all_data<-as.data.frame(full_data)

#update mi endpoint name
setnames(all_data,"mi","mutual.information")

#QC check. remove any NAs in the r column
na.omit(all_data$r)
...
set_parameters <- c("meanfiringrate","burst.per.min","nAE","nABE","per.spikes.in.burst","r","mutual.information") 

#save the wrangled data as a csv
write.csv(all_data,file = "C:/Users/sboyd01/OneDrive - Environmental Protection Agency (EPA)/Profile/Documents/Project-Chronic DNT Organoid/Data Analysis/Tipping Points/Exported_Files/New_Timepoints_CF/All_Data.csv")
...
normalize_by_plate <- function(all_data) {
 ...
 # scaled_plates[scaled_plates[,"R"] < 0, "R"] <- 0 # Set correlation value minimums to zero
  ...
  scaled_plates <- scaled_plates [!is.nan(scaled_plates$r),]
...
plot_dose_perturbations <- function(norm_data, parameter, compound) {
 ...
 # Plot with ggplot
  ggplot(trt_meds, aes(x=DIV, y=med, group=dose, color=log10(dose+1))) +
    ...
    theme(axis.text=element_text(size=4), axis.title.x=element_text(size=6), axis.title.y=element_text(size=0),  
    plot.title=element_text(size=6), legend.text=element_text(size=2), legend.position = "none") +
...
make_multi <- function(compound) {
  ...
  pdf(paste(compound, "_CF_data_2.pdf", sep=""))
...
Compounds<-"Dieldrin" #update for compound of interest
  ...
  for (i in cnd_reps) {
    ...
    trt_slopes[,"div5to7"] <- (subset(trt_slopes, DIV==7)[,"sum"][1] - subset(trt_slopes, DIV==5)[,"sum"][1])/7
...
ggsave(vis,filename=paste(Compounds,"_Total_Scalar_Perturbations_CF_data",".png",sep=""),height=5.01,width=5.32,unit="in")
  
  
  # Plot DIV5 to DIV26 change
...
sink(file=paste(Compounds,"_Tipping_Point_List_CF",".csv",sep=""), append=TRUE, split=TRUE)
...
vis_plot_list<-list(vis5_7,vis7_9,vis9_12)

vis_plots<-ggarrange(plotlist=vis_plot_list,ncol=3)
vis_plots

#ggsave
ggsave(plot=vis_plots,filename=paste(Compounds,"_Tipping_Point_Determination",".png",sep=""),bg="white",width=12,height=4,units="in")
