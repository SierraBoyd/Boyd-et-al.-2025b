---
title: "Calculation of Toxicological Tipping Points from Boyd et al., 2025 BrainSphere Data"
author: "Sierra Boyd"
date: "2025-03-27"
...
#Full join activity and network data together here
full_data<-rbind(Activity,Network)
...
full_data <-full_data%>%  filter(DOP != "28") 
...
setnames(full_data_wide,"activeArea","Active_Area")
setnames(full_data_wide,"ISI_mean","ISI")
setnames(full_data_wide,"BurstFreq","Burst_Frequency")
setnames(full_data_wide,"ClusterFreq","Cluster_Frequency")
setnames(full_data_wide,"CountsperCluster","Bursts_per_Cluster")
setnames(full_data_wide,"Duration","Cluster_Duration")
setnames(full_data_wide,"firingRate_mean","Firing_Rate")
setnames(full_data_wide,"amplitude_mean","Spike_Amplitude")
setnames(full_data_wide,"BurstPeak_Mean","Burst_Peak")
setnames(full_data_wide,"SPB_Mean","Spikes_per_Burst")
setnames(full_data_wide,"burstDur","Burst_Duration")
...
full_data_wide<-full_data_wide%>%select(c("date","Plate.SN","DIV","well","trt","dose","units","Active_Area","Firing_Rate","Burst_Peak","ICI","Cluster_Duration"))
...
set_parameters <- c("Active_Area","Firing_Rate","Burst_Peak","ICI","Cluster_Duration") 

#save the wrangled data as a csv
write.csv(all_data,file = "C:/Users/sboyd01/OneDrive - Environmental Protection Agency (EPA)/Profile/Documents/Project-Chronic DNT Organoid/Data Analysis/Tipping Points/Exported_Files/Final_Endpoints_Activity_2BMAD/All_Data.csv")
...
Compounds<-"Dieldrin" #update for compound of interest
...
set_parameters <- c("Active_Area","Firing_Rate","Burst_Peak","ICI","Cluster_Duration") 
    ...
    trt_slopes[,"div5to7"] <- (subset(trt_slopes, DIV==7)[,"sum"][1] - subset(trt_slopes, DIV==5)[,"sum"][1])/2
    trt_slopes[,"div7to9"] <- (subset(trt_slopes, DIV==9)[,"sum"][1] - subset(trt_slopes, DIV==7)[,"sum"][1])/2
    trt_slopes[,"div9to12"] <- (subset(trt_slopes, DIV==12)[,"sum"][1] - subset(trt_slopes, DIV==9)[,"sum"][1])/3
    trt_slopes[,"div12to14"] <- (subset(trt_slopes, DIV==14)[,"sum"][1] - subset(trt_slopes, DIV==12)[,"sum"][1])/2
    trt_slopes[,"div14to16"] <- (subset(trt_slopes, DIV==16)[,"sum"][1] - subset(trt_slopes, DIV==14)[,"sum"][1])/2
    trt_slopes[,"div16to19"] <- (subset(trt_slopes, DIV==19)[,"sum"][1] - subset(trt_slopes, DIV==16)[,"sum"][1])/3
    trt_slopes[,"div19to21"] <- (subset(trt_slopes, DIV==21)[,"sum"][1] - subset(trt_slopes, DIV==19)[,"sum"][1])/2
    trt_slopes[,"div21to23"] <- (subset(trt_slopes, DIV==23)[,"sum"][1] - subset(trt_slopes, DIV==21)[,"sum"][1])/2
    trt_slopes[,"div23to26"] <- (subset(trt_slopes, DIV==26)[,"sum"][1] - subset(trt_slopes, DIV==23)[,"sum"][1])/3
  ...
  final_var <- subset(cntrl_var, DIV==26)[,"max"][1]
...
ggsave(vis,filename=paste(Compounds,"_Total_Scalar_Perturbations_2",".png",sep=""),height=5.01,width=5.32,unit="in")
  ...
  vis5_7 <- ggplot(trt_derivs, aes(x=log10(dose))) +
    theme_minimal() +
    geom_hline(yintercept=final_var, linetype=2) +
    geom_hline(yintercept=0, linetype=2) +
    geom_smooth(aes(y=sum), size=1.4, se=TRUE, span=0.8, col="blue", fill="blue") +
    geom_smooth(aes(y=div5to7*5), size=1.4, se=TRUE, span=0.8, col="forestgreen", fill="forestgreen") +
    geom_point(aes(y=sum), col="blue", position=position_jitter(w=0.2, h=0.25)) +
    geom_point(aes(y=div5to7*5), col="forestgreen", position=position_jitter(w=0.2, h=0.25)) +
   ...
   vis7_9 <- ggplot(trt_derivs, aes(x=log10(dose))) +
    theme_minimal() +
    geom_hline(yintercept=final_var, linetype=2) +
    geom_hline(yintercept=0, linetype=2) +
    geom_smooth(aes(y=sum), size=1.4, se=TRUE, span=0.8, col="blue", fill="blue") +
    geom_smooth(aes(y=div7to9*5), size=1.4, se=TRUE, span=0.8, col="forestgreen", fill="forestgreen") +
    geom_point(aes(y=sum), col="blue", position=position_jitter(w=0.2, h=0.25)) +
    geom_point(aes(y=div7to9*5), col="forestgreen", position=position_jitter(w=0.2, h=0.25)) +
    coord_cartesian(ylim=c(-50,50)) +
    ggtitle("DIV7-9") +
    theme(axis.text=element_text(size=12), axis.title.x=element_text(size=12), axis.title.y=element_text(size=0),   plot.title=element_text(size=22), legend.text=element_text(size=12))+
    theme(plot.title = element_text(hjust = 0.5))   
    ...
    vis9_12 <- ggplot(trt_derivs, aes(x=log10(dose))) +
    theme_minimal() +
    geom_hline(yintercept=final_var, linetype=2) +
    geom_hline(yintercept=0, linetype=2) +
    geom_smooth(aes(y=sum), size=1.4, se=TRUE, span=0.8, col="blue", fill="blue") +
    ...
    vis12_14 <- ggplot(trt_derivs, aes(x=log10(dose))) +
    theme_minimal() +
    geom_hline(yintercept=final_var, linetype=2) +
    geom_hline(yintercept=0, linetype=2) +
    geom_smooth(aes(y=sum), size=1.4, se=TRUE, span=0.8, col="blue", fill="blue") +
    geom_smooth(aes(y=div12to14*5), size=1.4, se=TRUE, span=0.8, col="forestgreen", fill="forestgreen") +
    geom_point(aes(y=sum), col="blue", position=position_jitter(w=0.2, h=0.25)) +
    geom_point(aes(y=div12to14*5), col="forestgreen", position=position_jitter(w=0.2, h=0.25)) +
    coord_cartesian(ylim=c(-50,50)) +
    ggtitle("DIV12-14") +
    theme(axis.text=element_text(size=12), axis.title.x=element_text(size=12), axis.title.y=element_text(size=0),   plot.title=element_text(size=22), legend.text=element_text(size=12))+
    theme(plot.title = element_text(hjust = 0.5)) 
    ...
    vis14_16 <- ggplot(trt_derivs, aes(x=log10(dose))) +
    theme_minimal() +
    geom_hline(yintercept=final_var, linetype=2) +
    geom_hline(yintercept=0, linetype=2) +
    geom_smooth(aes(y=sum), size=1.4, se=TRUE, span=0.8, col="blue", fill="blue") +
    geom_smooth(aes(y=div14to16*5), size=1.4, se=TRUE, span=0.8, col="forestgreen", fill="forestgreen") +
    geom_point(aes(y=sum), col="blue", position=position_jitter(w=0.2, h=0.25)) +
    geom_point(aes(y=div14to16*5), col="forestgreen", position=position_jitter(w=0.2, h=0.25)) +
    coord_cartesian(ylim=c(-50,50)) +
    ggtitle("DIV14-16") +
    theme(axis.text=element_text(size=12), axis.title.x=element_text(size=12), axis.title.y=element_text(size=0),   plot.title=element_text(size=22), legend.text=element_text(size=12))+
    theme(plot.title = element_text(hjust = 0.5))   
    ...
    vis16_19 <- ggplot(trt_derivs, aes(x=log10(dose))) +
    theme_minimal() +
    geom_hline(yintercept=final_var, linetype=2) +
    geom_hline(yintercept=0, linetype=2) +
    geom_smooth(aes(y=sum), size=1.4, se=TRUE, span=0.8, col="blue", fill="blue") +
    geom_smooth(aes(y=div16to19*5), size=1.4, se=TRUE, span=0.8, col="forestgreen", fill="forestgreen") +
    geom_point(aes(y=sum), col="blue", position=position_jitter(w=0.2, h=0.25)) +
    geom_point(aes(y=div16to19*5), col="forestgreen", position=position_jitter(w=0.2, h=0.25)) +
    coord_cartesian(ylim=c(-50,50)) +
    ggtitle("DIV16-19") +
    theme(axis.text=element_text(size=12), axis.title.x=element_text(size=12), axis.title.y=element_text(size=0),   plot.title=element_text(size=22), legend.text=element_text(size=12))+
    theme(plot.title = element_text(hjust = 0.5))    
    ...
    vis19_21 <- ggplot(trt_derivs, aes(x=log10(dose))) +
    theme_minimal() +
    geom_hline(yintercept=final_var, linetype=2) +
    geom_hline(yintercept=0, linetype=2) +
    geom_smooth(aes(y=sum), size=1.4, se=TRUE, span=0.8, col="blue", fill="blue") +
    geom_smooth(aes(y=div19to21*5), size=1.4, se=TRUE, span=0.8, col="forestgreen", fill="forestgreen") +
    geom_point(aes(y=sum), col="blue", position=position_jitter(w=0.2, h=0.25)) +
    geom_point(aes(y=div19to21*5), col="forestgreen", position=position_jitter(w=0.2, h=0.25)) +
    coord_cartesian(ylim=c(-50,50)) +
    ggtitle("DIV19-21") +
    theme(axis.text=element_text(size=12), axis.title.x=element_text(size=12), axis.title.y=element_text(size=0), 
          plot.title=element_text(size=22), legend.text=element_text(size=12))+
    theme(plot.title = element_text(hjust = 0.5))  
  

  vis21_23 <- ggplot(trt_derivs, aes(x=log10(dose))) +
    theme_minimal() +
    geom_hline(yintercept=final_var, linetype=2) +
    geom_hline(yintercept=0, linetype=2) +
    geom_smooth(aes(y=sum), size=1.4, se=TRUE, span=0.8, col="blue", fill="blue") +
    geom_smooth(aes(y=div21to23*5), size=1.4, se=TRUE, span=0.8, col="forestgreen", fill="forestgreen") +
    geom_point(aes(y=sum), col="blue", position=position_jitter(w=0.2, h=0.25)) +
    geom_point(aes(y=div21to23*5), col="forestgreen", position=position_jitter(w=0.2, h=0.25)) +
    coord_cartesian(ylim=c(-50,50)) +
    ggtitle("DIV21-23") +
    theme(axis.text=element_text(size=12), axis.title.x=element_text(size=12), axis.title.y=element_text(size=0),   plot.title=element_text(size=22), legend.text=element_text(size=12))+
    theme(plot.title = element_text(hjust = 0.5))   
    ...
    vis23_26 <- ggplot(trt_derivs, aes(x=log10(dose))) +
    theme_minimal() +
    geom_hline(yintercept=final_var, linetype=2) +
    geom_hline(yintercept=0, linetype=2) +
    geom_smooth(aes(y=sum), size=1.4, se=TRUE, span=0.8, col="blue", fill="blue") +
    geom_smooth(aes(y=div23to26*5), size=1.4, se=TRUE, span=0.8, col="forestgreen", fill="forestgreen") +
    geom_point(aes(y=sum), col="blue", position=position_jitter(w=0.2, h=0.25)) +
    geom_point(aes(y=div23to26*5), col="forestgreen", position=position_jitter(w=0.2, h=0.25)) +
    coord_cartesian(ylim=c(-50,50)) +
    ggtitle("DIV23-26") +
    theme(axis.text=element_text(size=12), axis.title.x=element_text(size=12), axis.title.y=element_text(size=0),   plot.title=element_text(size=22), legend.text=element_text(size=12))+
    theme(plot.title = element_text(hjust = 0.5))   
...
add_deriv(vis5_7, "div5to7")
add_deriv(vis7_9, "div7to9")
add_deriv(vis9_12, "div9to12")
add_deriv(vis12_14, "div12to14")
add_deriv(vis14_16, "div14to16")
add_deriv(vis16_19, "div16to19")
add_deriv(vis19_21, "div19to21")
add_deriv(vis21_23, "div21to23")
add_deriv(vis23_26, "div23to26")
  sink()
    

 vis5_7<- add_deriv(vis5_7, "div5to7")
 vis7_9<- add_deriv(vis7_9, "div7to9")
 vis9_12<- add_deriv(vis9_12, "div9to12")
 vis12_14<- add_deriv(vis12_14, "div12to14")
 vis14_16<- add_deriv(vis14_16, "div14to16")
 vis16_19<- add_deriv(vis16_19, "div16to19")
 vis19_21<- add_deriv(vis19_21, "div19to21")
 vis21_23<- add_deriv(vis21_23, "div21to23")
 vis23_26<- add_deriv(vis23_26, "div23to26")


library(ggpubr)

vis_plot_list<-list(vis5_7,vis7_9,vis9_12,vis12_14,vis14_16,vis16_19,vis19_21,vis21_23,vis23_26)
